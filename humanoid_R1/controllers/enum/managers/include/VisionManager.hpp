...
namespace managers {
  ...
  class VisionManager {
  public:
    VisionManager(int width, int height, int hue, int hueTolerance, int minSaturation, int minValue, int minPercent,
                            int maxPercent);
    virtual ~VisionManager();

    bool getBallCenter(double &x, double &y, const unsigned char *image);
    double getLinePosition(const unsigned char *image, int width, int height);
