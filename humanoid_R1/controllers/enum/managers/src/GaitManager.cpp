...
GaitManager::GaitManager(webots::Robot *robot, const std::string &iniFilename) :
  mRobot(robot),
  ...
  mIsWalking(false) {
  if (!mRobot) {
    cerr << "GaitManager: The robot instance is required" << endl;
...
void GaitManager::step(int step) {
  if (step < 8) {
    cerr << "GaitManager: steps of less than 8ms are not supported" << endl;
    return;
  }
#ifdef CROSSCOMPILATION
  ...
  if (mBalanceEnable && (mRobot->getGyro("Gyro")->getSamplingPeriod() <= 0)) {
    cerr << "The Gyro is not enabled. GaitManager need the Gyro to run the balance algorithm. The Gyro will be "
...
void GaitManager::start() {
  mIsWalking = true;
  mWalking->Start();
}

#ifndef CROSSCOMPILATION
double GaitManager::valueToPosition(unsigned short value) {
  double degree = MX28::Value2Angle(value);
  double position = degree / 180.0 * M_PI;
  return position;
}

void GaitManager::myStep() {
