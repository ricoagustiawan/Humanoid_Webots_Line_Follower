...
MotionPlayManager::MotionPlayManager(webots::Robot *robot, const std::string &customMotionFile) :
  mRobot(robot),
  mCorrectlyInitialized(true) {
  if (!mRobot) {
    cerr << "MotionPlayManager: The robot instance is required" << endl;
...
#ifdef CROSSCOMPILATION
  MotionTimerManager::MotionTimerInit();
  ...
  if (0 < firm_ver && firm_ver < 27)
    filename = "managers/src/Data/motion_1024.bin";
  else if (27 <= firm_ver)
    filename = "managers/src/Data/motion_4096.bin";
  else {
  ...
  if (mAction->LoadFile(const_cast<char *>(path)) == false) {
    cerr << "MotionPlayManager: Cannot load the motion from " << filename << endl;
    mCorrectlyInitialized = false;
    mAction = NULL;
    return;
  }
...
MotionPlayManager::~MotionPlayManager() {
  if (mAction && mAction->IsRunning())
    mAction->Stop();
}

void MotionPlayManager::playPage(int id, bool sync) {
...
#ifndef CROSSCOMPILATION
void MotionPlayManager::myStep() {
  int ret = mRobot->step(mBasicTimeStep);
  if (ret == -1)
    exit(EXIT_SUCCESS);
}

void MotionPlayManager::wait(int duration) {
...
void MotionPlayManager::achieveTarget(int timeToAchieveTarget) {
  ...
  for (int i = 0; i < DMM_NMOTORS; i++) {
    if (mPositionSensors[i]->getSamplingPeriod() <= 0) {
      cerr << "The position feedback of the position sensor " << motorNames[i]
           << "S is not enabled. MotionPlayManager need to read the position of all position sensors. The position will "
...
double MotionPlayManager::valueToPosition(unsigned short value) {
  double degree = MX28::Value2Angle(value);
  double position = degree / 180.0 * M_PI;
  return position;
}

void MotionPlayManager::InitMotionAsync() {
  bool stepNeeded = false;
  for (int i = 0; i < DMM_NMOTORS; i++) {
    if (mPositionSensors[i]->getSamplingPeriod() <= 0) {
      cerr << "The position feedback of the position sensor " << motorNames[i]
...
void MotionPlayManager::step(int duration) {
...
void *MotionPlayManager::MotionThread(void *param) {
  MotionPlayManager *instance = (dynamic_cast<MotionPlayManager *>(param));
